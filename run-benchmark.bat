...
echo ===================================================
echo  Building FastStylus ^& JMH Benchmarks Uber-Jar
echo ===================================================

call mvn -q clean install -DskipTests 2>nul
if %ERRORLEVEL% NEQ 0 (
    echo [ERROR] FastStylus install failed!
    pause
    exit /b %ERRORLEVEL%
)
...
echo ===================================================
java --enable-native-access=ALL-UNNAMED --sun-misc-unsafe-memory-access=allow "-Djava.library.path=build;src\main\resources\native;target\classes" -jar target\benchmarks.jar -f 1 -wi 2 -i 3 -tu ms -bm thrpt
pause
